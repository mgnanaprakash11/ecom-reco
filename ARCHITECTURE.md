...
This monorepo delivers an end-to-end ecommerce reconciliation platform composed of two Next.js applications, shared TypeScript packages, a Vercel Workflows pipeline for asynchronous ingestion, and a dbt analytics layer running on Supabase Postgres. Turborepo and pnpm orchestrate builds across the workspace.
...
- **Background Workflows (`apps/web-app/app/workflows`)**: Vercel Workflows defined alongside shared helpers ingest raw CSV uploads, persist records with Drizzle ORM, and dispatch analytics workflows.
...
3. **Workflow Dispatch**: The Vercel Workflows SDK enqueues `process-orders-workflow` with the tenant, batch, and object path.
4. **Parsing & Persistence**: The workflow downloads the CSV, parses rows (`apps/web-app/lib/workflows/process-orders-upload.ts`), and writes canonicalised JSON payloads into `raw.orders` alongside structured Shopify records in `raw.shopify_orders`. Batch status transitions (`received → parsing → loaded`) are updated in Postgres.
5. **Analytics Trigger**: Upon successful load, the workflow dispatches the GitHub Actions workflow (`run-dbt.yml`) to execute the dbt project. Workflow metadata is echoed back into `data_upload_batches.metadata.githubWorkflow`.
...
7. **Consumption**: Future dashboard surfaces and downstream automations query the marts layer, while additional workflows can schedule follow-on tasks based on dbt outputs.
...
- **Supabase Postgres**: Acts as the primary OLTP and analytics store. Service-role keys are required for both web actions and Vercel Workflows; user-facing requests authenticate via Supabase auth helpers. Object storage hosts raw CSV uploads.
...
- **Vercel Workflows**: Defined in `apps/web-app/app/workflows`, the workflows are dispatched via the `workflow/api` helper. They handle chunked inserts, idempotency (replace-by-batch), and GitHub workflow dispatch with retry-friendly errors.
- **GitHub Actions**: The `run-dbt.yml` workflow runs `dbt build` using repository secrets (`DBT_*`, `DATABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`). Workflow metadata persists contextual payloads (batch id, tenant id) for auditability.
...
- **Testing**: Vitest + React Testing Library for UI, with emphasis on `/upload` flows and workflow transitions. dbt tests should validate data quality in marts.

## Local Development Topology
- Run the main surfaces with `pnpm dev` (web + docs).
- Supabase credentials must be present in `.env.local` for apps and in the shell for dbt (`DBT_HOST`, etc.).
- Workflows execute on Vercel; for local dry-runs call `processOrdersUpload` directly or invoke the workflow via the `workflow/api` helpers.
